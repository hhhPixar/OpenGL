 ...
 * KeyCodes.h —— 键盘按键码定义
 *
 * 这里把 GLFW(glfw3.h)里的按键常量原样搬过来,改了个前缀 HZ_(源自 Hazel
 * 教程),值和 GLFW_KEY_* 完全一样(32 是空格,65 是 A……)。
 *
 * 为什么不直接 #include <GLFW/glfw3.h> 用 GLFW_KEY_A?
 *   - 因为这是平台无关层(Core),不该依赖具体平台库 GLFW。用一个独立的键码
 *     头文件,上层只认 HZ_KEY_A 这些抽象常量;具体平台实现(WindowsInput)
 *     再把 HZ_KEY_A 当作 GLFW 的键码传给 glfwGetKey(因为数值相等,可直接用)。
 *
 * 宏(#define)是预处理指令:编译前会把所有 HZ_KEY_A 替换成字面量 65。
 * 这里用宏而不用 enum,是为了和 GLFW 的 #define 常量完全对应、可直接当
 * 整数传给 GLFW 的 C 函数。
 * ========================================================================== */
